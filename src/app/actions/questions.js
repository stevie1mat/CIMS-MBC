...
const getQuestionTypeString = (index) => {
  const map = {
...
export async function uploadQuestions(quizId, formData) {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) { // Skip header row (index 0) if there is one. Wait, legacy might not have header. We'll check if row 0 looks like a header.
      ...
      if (options.length > 0) {
        ...
        if (optError) console.error('Error inserting options:', optError)
      }
    ...
    return { error: 'Failed to process Excel file. Please ensure it follows the correct format.' }
  }
}
...
export async function deleteQuestion(id, quizId) {
  const supabase = await createClient()
