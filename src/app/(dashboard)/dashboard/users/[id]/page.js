  ...
  return (
    <div>
      <div className={styles.panelHeader} style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <EditUserForm user={profile} groups={groups} accountTypes={accountTypes} />
      </div>
      ...
      <div className={styles.panel} style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '2rem', padding: '2rem' }}>
        <img 
          src={gravatarUrl} 
          alt="Profile Avatar" 
          style={{ width: '100px', height: '100px', borderRadius: '50%', border: '4px solid #f8fafc', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
        />
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div className={styles.panel}>
          ...
          <div className={styles.panelBody} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '0.5rem', borderBottom: '1px solid #f1f5f9' }}>
              <span style={{ color: '#64748b', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Users size={18}/> Group</span>
              <span style={{ fontWeight: 500, color: '#0f172a' }}>{profile.group_name}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '0.5rem', borderBottom: '1px solid #f1f5f9' }}>
      ...
      <div className={styles.panel} style={{ marginBottom: '2rem' }}>
        <div className={styles.panelHeader}>
          <h3 className={styles.panelTitle} style={{ fontSize: '1.2rem' }}>Category Proficiency</h3>
        </div>
          ...
          <table className={styles.table} style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                ...
                <th style={{ padding: '1rem', color: '#475569', fontWeight: 600 }}>Overall Percentage</th>
                <th style={{ padding: '1rem', color: '#475569', fontWeight: 600 }}>Percentage Last Quiz</th>
              </tr>
            </thead>
            <tbody>
              {categories.length === 0 ? (
                <tr><td colSpan="3" style={{ padding: '1rem', textAlign: 'center', color: '#64748b' }}>No category data available.</td></tr>
              ) : (
                categories.map((cat, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: cat.overallPercentage <= 50 ? '#fef2f2' : '#f0fdf4' }}>
                    <td style={{ padding: '1rem', fontWeight: 500 }}>{cat.categoryName}</td>
                    <td style={{ padding: '1rem' }}>{cat.overallPercentage.toFixed(0)}%</td>
                    <td style={{ padding: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      {cat.recentPercentage >= cat.overallPercentage ? (
                        <TrendingUp size={16} color="#16a34a" title="Improving" />
                      ) : (
                        <TrendingDown size={16} color="#dc2626" title="Needs Improvement" />
                      )}
                      {cat.recentPercentage.toFixed(0)}%
                    </td>
                  </tr>
                ))
              )}
      ...
      <div className={styles.panel} style={{ marginBottom: '2rem' }}>
        <div className={styles.panelHeader}>
          <h3 className={styles.panelTitle} style={{ fontSize: '1.2rem' }}>Questions Incorrect</h3>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table className={styles.table} style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                <th style={{ padding: '1rem', color: '#475569', fontWeight: 600, width: '80px' }}>ID</th>
                <th style={{ padding: '1rem', color: '#475569', fontWeight: 600 }}>Question</th>
                <th style={{ padding: '1rem', color: '#475569', fontWeight: 600, width: '100px' }}>Action</th>
              </tr>
            </thead>
            <tbody>
              {incorrectQuestions.length === 0 ? (
                <tr><td colSpan="3" style={{ padding: '1rem', textAlign: 'center', color: '#64748b' }}>No incorrect questions found.</td></tr>
              ) : (
                incorrectQuestions.map((q, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '1rem' }}>{q.id}</td>
                    <td style={{ padding: '1rem', color: '#334155' }}>
                      <div dangerouslySetInnerHTML={{ __html: q.body.substring(0, 100) + (q.body.length > 100 ? '...' : '') }} />
                    </td>
                    <td style={{ padding: '1rem' }}>
                      <button className={styles.btnOutline} style={{ padding: '0.25rem 0.5rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                        <Eye size={16} /> View
                      </button>
                    </td>
                  </tr>
